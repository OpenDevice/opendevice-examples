...
public class SectorsActivity extends SherlockFragmentActivity
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        registerReceiver(connectionStateReceiver, new IntentFilter(IntelliHouseIntent.CONNECTION_CHANGE));
    ...
    private BroadcastReceiver connectionStateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if(manager == null){
                manager = BaseDeviceManager.getInstance();
                manager.addConnectionListener(SectorsActivity.this);
                try {
                    BaseDeviceManager.getInstance().send(new GetDevicesRequest(), true);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            ...
            unregisterReceiver(this); // requied only one time...
        }
    };
...
//					tfName.setTextColor((device.getValue() == 1 ? Color.BLACK: Color.GREEN));	
    ...
    public void onMessageReceived(Message message, DeviceConnection deviceConnection) {
        ...
        if(message instanceof GetDevicesResponse){
            // FIXME: Codigo fixo e nao reutilizavel... deve ser implementado a logica de separacao e cadastro de devices
            Sector sector = sectors.get(0);

            if(sector.getDevices().isEmpty()){

                Collection<Device> devices = BaseDeviceManager.getInstance().getDevices();

                sector.getDevices().addAll(devices);

                for (DeviceListFragment fragment : fragments.values()) {
                    if (fragment.getSector().getName().equals(sector.getName())) {
                        fragment.updateDeviceStates();
                    }
